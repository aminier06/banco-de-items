...
authRoutes.post("/login", async (c) => {
  ...
  if (!correo || !password) {
    return c.json({ error: "Correo y contrasena son obligatorios." }, 400);
  }
  ...
  if (!ok) {
    await registrar({ accion: "LOGIN_FALLIDO", detalle: { correo: user.correo, motivo: "Contrasena incorrecta" }, ip, usuario: { id: user.id, nombre: user.nombre, correo: user.correo } });
    return c.json({ error: "Correo o contrasena incorrectos." }, 401);
  }
...
authRoutes.post("/change-password", requerirSesion, async (c) => {
  ...
  if (!passwordActual || !passwordNueva) {
    return c.json({ error: "Indica tu contrasena actual y la nueva." }, 400);
  }
  if (passwordNueva.length < 4) {
    return c.json({ error: "La nueva contrasena debe tener al menos 4 caracteres." }, 400);
  }
  ...
  await registrar({ accion: "RESETEAR_PASSWORD", entidad: "users", entidad_id: user.id, usuario: { id: user.id, nombre: user.nombre, correo: user.correo }, detalle: { accion: "Cambio de contrasena propio" }, ip: obtenerIp(c) });
